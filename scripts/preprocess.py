...
def preprocess(args: argparse.Namespace):
    ...
    for lang in args.languages:
        ...
        for subject, task in product(subjects, tasks[lang]):
            ...
            if os.path.exists(subj_task_path):
                ...
                if args.output_single:
                    df_metrics.to_csv(
                        os.path.join(lang_output_paths[lang], f"{subject}_{task}.tsv"), sep="\t", index=False
                    )
                    if args.output_texts:
                        df_texts.to_csv(
        ...
        if args.output_merged_subjects:
            ...
            for task, dfs in texts_dict[lang].items():
                ...
                df_texts.to_csv(
                    os.path.join(merged_dir, f"{lang}_{'_'.join(subjects)}_{task}_texts.tsv"), sep="\t", index=False
                )
    if args.output_merged_languages:
        ...
        for task in all_tasks:
            ...
            df_texts.to_csv(os.path.join(merged_dir, f"full_{task}_texts.tsv"), sep="\t", index=False)
